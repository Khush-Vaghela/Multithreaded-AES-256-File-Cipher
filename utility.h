...
static void safe_path_join(char *dest, size_t dest_size, const char *base, const char *name) {
    if(dest_size == 0) {
        return;
    }
    dest[0] = '\0';
    ...
    if(base[0] == '/' && base[1] == '\0') {
        // dest = "/" + name (skip leading '/' in name if present)
    ...
    // if base has trailing slash, remove it for uniformity
